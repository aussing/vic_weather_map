...
class Observations:
    ...
    def wind_drection_deg(self, wmo_id=None):
        """ 
        """
        ...
        if elements is not None:
            wind_direction_deg = elements.find('element', {'type': 'wind_dir_deg'})

            if wind_direction_deg is not None and len(wind_direction_deg.contents) > 0:
                    return wind_direction_deg.contents[0]

        return None
    
    def wind_speed(self, wmo_id=None):
        """ 
        """
        ...
        if elements is not None:
            wind_spd_kmh = elements.find('element', {'type': 'wind_spd_kmh'})

            if wind_spd_kmh is not None and len(wind_spd_kmh.contents) > 0:
